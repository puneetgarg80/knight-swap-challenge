...
const Square: React.FC<SquareProps> = React.memo(({ name, piece, onClick, isSelected, isPossibleMove }) => {
  ...
  const ringColor = isSelected ? 'ring-4 ring-offset-2 ring-offset-gray-800 ring-cyan-600' : '';
  
  // Dark text for high visibility on white background
  const labelColor = 'text-black';

  return (
    <div
      onClick={() => onClick(name)}
      className={`relative flex items-center justify-center aspect-square rounded-sm cursor-pointer border-2 border-black transition-all duration-200 ${bgColor} ${hoverBgColor} ${ringColor}`}
    >
      {isPossibleMove && (
